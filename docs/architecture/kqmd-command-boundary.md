...
- owned `query` plain default path는 explicit `intent`, explicit `--candidate-limit`, explicit collection filter가 없을 때 cost-capped fast-default lexical retrieval을 우선하고, advanced surface만 upstream-compatible helper path를 유지한다
