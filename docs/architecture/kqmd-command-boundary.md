...
4. `execution`
   ...
   `search/query/update/embed/status/mcp/bench`는 더 이상 fixed scaffold stderr만 반환하지 않고,
   owned runtime과 formatter layer를 거쳐 동작한다. `mcp`는 local MCP server boundary를 소유하고,
   `bench`는 upstream-shaped compare benchmark surface를 local ownership으로 소유하며,
...
- owned `query`는 `QueryExecutionPlan`을 통해 fast-default(cost-capped structured original lex+vec)와 compatibility path를 명시적으로 나누고, execution truth는 `QueryExecutionSummary.execution`에서 공통으로 노출한다
...
- owned `bench`는 upstream `BenchmarkFixture` core schema를 기준선으로 삼되, 결과 표면은 upstream QMD와 current K-QMD compare semantics를 local ownership으로 소유한다
